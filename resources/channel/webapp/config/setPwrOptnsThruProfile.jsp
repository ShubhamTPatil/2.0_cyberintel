...
function selectFile() {
    profilePwrOptnsForm.file.click();
}
...
function setChecked(val) {
    ...
    var colForm = document.profilePwrOptnsForm;
...
function processEdit() {
    ...
    for (i = 0; i < len; i++) {
        ...
        if ((currentElement.indexOf(chkBoxNamePrefix)) > -1) {
            ...
            if (colForm.elements[i].checked) {
                ...
                document.profilePwrOptnsForm.name = currentElement.substring(chkBoxNamePrefix.length());
    ...
    saveState(document.profilePwrOptnsForm, '/profilePowerOptions.do?action=edit')
...
%>
<webapps:helpContext context="sm" topic="power_profiles"/>
...
<html:form method="post" name="profilePwrOptnsForm" enctype="multipart/form-data" action="/profilePowerOptions.do" type="com.marimba.apps.subscriptionmanager.webapp.forms.ProfilePwrOptnsForm" >
...
<logic:equal name="profilePwrOptnsForm" property="status" value="Export-Failed">
    <div class="statusMessage" id="critical" width="90%">
        ...
        <p align="left"><webapps:pageText key="export.warning.message"/>
            <bean:write name="profilePwrOptnsForm" property="statusDesc"/>
        </p>
    </div>
</logic:equal>

<logic:equal name="profilePwrOptnsForm" property="status" value="Import-Failed">
    <div class="statusMessage" id="critical" width="90%">
...
<logic:equal name="profilePwrOptnsForm" property="status" value="Import-Succeeded">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr valign="middle" class="smallButtons">
            <td nowrap class="tableRowActions">
                <input disabled name="removeBtn" id="removeBtn" type="button" value="<webapps:pageText key="remove" type="button" shared="true"/>" onClick="javascript:saveState(document.profilePwrOptnsForm, '/profilePowerOptions.do?action=remove');">
            </td>
            <td nowrap class="tableRowActions">
                <input disabled name="editBtn" id="editBtn" type="button" value="<webapps:pageText key="edit" type="button" shared="true"/>" onClick="javascript:saveState(document.profilePwrOptnsForm, '/profilePowerOptions.do?action=edit');">
            </td>
            <td nowrap class="tableRowActions">
                <input type="button" name="importbtn" value=" <webapps:pageText key="import" type="button" /> " onClick="fileImport.style.display=''">
            </td>
            <td nowrap class="tableRowActions">
                <input type="button" name="exportbtn" disabled="true" value=" <webapps:pageText key="export" type="button" /> " onClick="javascript:saveState(document.profilePwrOptnsForm, '/profilePowerOptions.do?action=export');">
            </td>

            <td nowrap class="tableRowActions" width="100%" align="right">
                <input name="addBtn" id="addBtn" type="button" value="<webapps:pageText key="add" type="button" shared="true"/>" onClick="javascript:saveState(document.profilePwrOptnsForm, '/profilePowerOptions.do?action=add');">
            </td>
        </tr>
        <tr>
            <td nowrap class="tableRowActions" colspan="5">
                <div id="fileImport" style="display:none"><webapps:pageText key="import.desc"/>
                    <html:file property="file" onchange="javascript:saveState(document.profilePwrOptnsForm,'/profilePowerOptions.do?action=import');" value=" <webapps:pageText key='export'/>"/>
                </div>
            </td>
        </tr>
    </table>
...
<!--onchange="javascript:saveState(document.profilePwrOptnsForm,'/profilePowerOptions.do?action=import');" />-->
<div id="pageNav">
    <input type="button" class="mainBtn" name="done" value=" <webapps:pageText key="done" shared="true" type="button" /> " onClick="javascript:saveState(document.profilePwrOptnsForm, '/config.do');">
</div>
</div>
</div>
</div>
