...
function  WWHHelp_Object(ParamURL)
{
  ...
  // Remedy additions
  this.fRemedyShowTitle                = Remedy_ShowTitle;

  // Load up messages
  // Remedy modification get the local as specified by the user.
...
// Function added by Remedy: displays the title of the current section in the RemedyTitleFrame.
function  Remedy_ShowTitle()
{
   VarFrame = eval(this.fGetFrameReference("RemedyTitleFrame"));
   RemedyTitleFrame = VarFrame.document;
   RemedyTitleFrame.open();

   RemedyTitleFrame.write('<html>');
   RemedyTitleFrame.write('<head><title>RemedyTitle.htm</title>');
   RemedyTitleFrame.write('<link rel="StyleSheet" href="../../../../catalog.css" type="text/css" />');
   RemedyTitleFrame.write('</head><body bgcolor="#F7F7F7" background="../images/BMCTitleBg.gif"');
   RemedyTitleFrame.write(' leftmargin="2" rightmargin="2" topmargin="2" bottommargin="0" ');
   RemedyTitleFrame.write(' marginwidth="2" marginheight="2">');
   RemedyTitleFrame.write('<p class="pRemedyTitle">'+WWHFrame.WWHHelp.mRemedyTitle+'</p></body></html>');
   RemedyTitleFrame.close();
}
...
function  WWHHelp_GetFrameReference(ParamFrameName)
{
  ...
  switch (ParamFrameName)
  {
    ...
    case "WWHDocumentFrame":
      ...
      //
      // Remedy change to add RemedyTitleFrame
      //VarFrameReference = this.fGetFrameReference("WWHContentFrame") + ".frames[1]";
      VarFrameReference = this.fGetFrameReference("WWHContentFrame") + ".frames[2]";
      break;

    // Remedy addition:
    case "RemedyTitleFrame":
      // WWHFrame.WWHContentFrame.RemedyTitleFrame
      //
      VarFrameReference = this.fGetFrameReference("WWHContentFrame") + ".frames[1]";
      break;
      ...
      (ParamStage == this.mInitStage))
  {
    ...
    switch (this.mInitStage)
    {
      ...
      case 5:  // Display document
        ...
        if (this.mbAccessible)
        {
          WWHFrame.WWHHelp.fSetFrameName("RemedyTitleFrame");
...
function  WWHHelp_DetermineContextDocument()
{
  ...
  else if (URLParams[0] != null)  // context specified
  {
    ...
    else  // Display splash page if nothing else found
    {
      ...
      ContextDocumentURL = this.mBaseURL + "FrontPage.htm";
    }
  }
  else  // Display splash page if nothing else found
  {
...
function  WWHHelp_GetSyncPrevNext(ParamHREF)
{
  ...
  if (BookFileHREF != null)
  {
    // NOTE FOR REMEDY: This one has to remain the way it is, otherwise 
    // the next link won't work.
...
function  WWHHelp_Update(ParamURL)
{
  ...
  else if (ParamURL.indexOf("FrontPage.htm") == -1)
  {
    // Try again in a bit
    //
...
function  WWHHelp_DocumentBookkeeping(ParamURL)
{
  ...
  if (ParamURL.indexOf("FrontPage.htm") == -1)
  {
