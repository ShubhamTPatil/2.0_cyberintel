...
<%
    ...
    Object profilesMap = session.getAttribute("scapprofilespropsmap");
%>
<webapps:helpContext context="spm" topic="scap" />
...
<script type="text/javascript">
    ...
    function alterProfileSettings(profileName, type) {
        var forward = '/scapSecurityLoad.do?selectedprofilename='+profileName+'&type='+type;
        send(document.scapSecurityForm, forward);
        return false;
    }

    function inactiveSCAPOptionParameters(optionSelected) {
        if ('exclude' == optionSelected) {
            $('#tr_scap_custom_data_table').hide();
            $('#tr_scap_standard_data_table').hide();
            document.scapSecurityForm["remediate"].disabled = true;
            ...
            $('#tr_scap_custom_data_table').show();
            $('#tr_scap_standard_data_table').hide();
            document.scapSecurityForm["remediate"].disabled = false;
			document.getElementById('customize_priority_id').disabled=false;
			document.getElementById('standard_priority_id').disabled=true;
        } else if ('standard' == optionSelected) {
            $('#tr_scap_custom_data_table').hide();
            $('#tr_scap_standard_data_table').show();
            document.scapSecurityForm["remediate"].disabled = false;
			document.getElementById('customize_priority_id').disabled=true;
    ...
    function switchContentProfileSettings(contentname) {
        var selectedSCAPOptionValue = document.scapSecurityForm.selectedSCAPOption.value;
        $.ajax({
    ...
    function showStandardSCAPProfileDetails(content, profile) {
        document.getElementById('iframeProfileInfo').src = '/spm/securitymgmt?command=gethtml&target=nonwindows&content=' + content + '&profile=' + profile;
        $('#standardprofiledetailswindow').modal('show');
    }
    function showCustomSCAPProfileDetails(fileName, profileid, templateName) {
        document.getElementById('iframeProfileInfo_custom').src = '/spm/securitymgmt?command=gethtml&target=nonwindows&content=' + fileName + '&profile=' + profileid + '&template=' + templateName + '.properties';
...
<html:hidden property="value(selectedScapAndProfilesNonWindows)"/>
...
<webapps:formtabs tabset="dist" tab="nonwindows" />
...
<tr id="tr_scap_custom_data_table">
    <td>&nbsp;</td>
    <td colspan="2" class="col3">
        <table id="scap_custom_data_table" class="table table-bordered table-striped dataTable">
            <thead>
            ...
            <tbody>
                <logic:iterate name="scapBeansListNonWindowsCustom" id="scapListCustom" scope="session" indexId="indexId">
                    <bean:define id="profile" name="scapListCustom" property="profiels" toScope="request"/>
                    <bean:define id="selectedProfile" name="scapListCustom" property="selectedProfile" toScope="request"/>
                    <tr>
                        <td>
                            <logic:equal name="scapListCustom" property="selected" value="true">
                                <input type="checkbox" id="chk1_<bean:write name="scapListCustom" property="fileName"/>" checked>
                            </logic:equal>
                            <logic:equal name="scapListCustom" property="selected" value="false">
                                <input type="checkbox" id="chk1_<bean:write name="scapListCustom" property="fileName"/>">
                            </logic:equal>
                        </td>
                        <td><bean:write name="scapListCustom" property="templateName"/></td>
                        <td><bean:write name="scapListCustom" property="title"/></td>
                        <td>
                        ...
                        <td><a href="#" onclick="showCustomSCAPProfileDetails('<bean:write name="scapListCustom" property="fileName"/>', $('#profilekey_custom_<%= indexId.toString() %>').val(), '<bean:write name="scapListCustom" property="templateName"/>');return false;"><img src="/spm/images/more-details.png" title="View Profile Details"/></a></td>
                    </tr>
                </logic:iterate>
            </tbody>
        </table>
...
<tr>
    <td width="8%" align="right" valign="top" style="padding-top:5px;padding-bottom:5px"> <html:radio property="selectedSCAPOption" value="standard" onclick="inactiveSCAPOptionParameters(this.value)"/>&nbsp; </td>
...
<tr id="tr_scap_standard_data_table">
    <td>&nbsp;</td>
    <td colspan="2" class="col3">
        <table id="scap_standard_data_table" class="table table-bordered table-striped dataTable">
            <thead>
                <td><input name="select_all" type="checkbox"></td>
                <td>Standard SCAP Content</td>
                <td>Profile Name</td>
            ...
            <tbody>
                <logic:iterate name="scapBeansListNonWindows" id="scapList" scope="session" indexId="indexId">
                    <bean:define id="profile" name="scapList" property="profiels" toScope="request"/>
                    ...
                    <tr>
                        <td>
                            <logic:equal name="scapList" property="selected" value="true">
                                <input type="checkbox" id="chk1_<bean:write name="scapList" property="fileName"/>" checked name="chk_box">
                            </logic:equal>
                            <logic:equal name="scapList" property="selected" value="false">
                                <input type="checkbox" id="chk1_<bean:write name="scapList" property="fileName"/>">
                            </logic:equal>
                        </td>
                        ...
                        <td><a href="#" onclick="showStandardSCAPProfileDetails('<bean:write name="scapList" property="fileName"/>', $('#profilekey_<%= indexId.toString() %>').val());return false;"><img src="/spm/images/more-details.png" title="View Profile Details"/></a></td>
                    </tr>
                </logic:iterate>
            </tbody>
        </table>
...
<tr>
    <td width="8%">&nbsp;</td>
    <td colspan="2"><%@ include file="/target/scap_setting_options.jsp" %></td>
</tr>
</table>
</div>
<!--end formContent-->
...
<div id="pageNav">
    <input name="Submit32" type="submit" class="mainBtn" onClick="javascript:saveState('/scapSecurityLoad.do?action=preview')" value="<webapps:pageText key="preview" type="button" shared="true"/>">
    &nbsp;
    <logic:present name="taskid">
...
var customSCAPDataTable;
var standardSCAPDataTable;

var optionSelected = document.scapSecurityForm.selectedSCAPOption.value;
populateCustomSCAPDataTable('#scap_custom_data_table');
persistCheckbox('#scap_custom_data_table');

populateStandardSCAPDataTable('#scap_standard_data_table');
persistCheckbox('#scap_standard_data_table');
...
function populateCustomSCAPDataTable(tableSelector) {
    customSCAPDataTable = $(tableSelector).DataTable();
    $(tableSelector + ' tbody').on('click', 'input[type="checkbox"]', function(e) {
        var $row = $(this).closest('tr');
        var data = customSCAPDataTable.row($row).data();
        var rowId = data[0];
    ...
    $('thead input[name="select_all"]', customSCAPDataTable.table().container()).on('click', function(e){
        if (this.checked){
...
function populateStandardSCAPDataTable(tableSelector) {
    standardSCAPDataTable = $(tableSelector).DataTable();

    $(tableSelector + ' tbody').on('click', 'input[type="checkbox"]', function(e) {
        var $row = $(this).closest('tr');
        var data = standardSCAPDataTable.row($row).data();
        var rowId = data[0];        
    ...
    $('thead input[name="select_all"]', standardSCAPDataTable.table().container()).on('click', function(e){
        if (this.checked){
            $(tableSelector + ' tbody input[type="checkbox"]:not(:checked)').trigger('click');
        } else {
            $(tableSelector + ' tbody input[type="checkbox"]:checked').trigger('click');
...
function getSelectedRowFromCustomSCAPDataTable(table) {
    return $(table.rows('.selected').data()).map(function(item, value) {
...
function saveState(forwardaction) {
    document.scapSecurityForm["value(forward)"].value = forwardaction;
    var optionSelected = document.scapSecurityForm.selectedSCAPOption.value;
    if ('customize' == optionSelected) {
        document.scapSecurityForm["value(selectedScapAndProfilesNonWindows)"].value = getSelectedRowFromCustomSCAPDataTable(customSCAPDataTable);
    } else if ('standard' == optionSelected) {
        document.scapSecurityForm["value(selectedScapAndProfilesNonWindows)"].value = getSelectedRowData(standardSCAPDataTable);
    }
