...
public class SecurityVulnerTgtViewAction extends AbstractAction implements IWebAppConstants, 
	ISubscriptionConstants, ComplianceConstants {
    ...
    protected Task createTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) {
        return new SecurityVulnerViewTask(mapping, form, request, response);
    }

    protected class SecurityVulnerViewTask extends SubscriptionDelayedTask {
    	SecurityVulnerViewTask(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) {
                ...
                if( target != null ){
                    ...
                    if (GRP_COMP_OVERVIEW_RSLT_TYPE.equals(view) && !target.getType().equals( TYPE_MACHINE )) {
                        ...
                        if( !hasReadPermission ) {
                            ...
                            display_results = SecurityLDAPUtils.getAssignedPolicy(SECURITY_SCAN_TYPE_OVAL, target.getName(), target.getType(), target.getId(), main, _user);
                            if( display_results != null && display_results.size() > 0 ) {
                                for (int i = 0; i<display_results.size(); i++) {
                                    ...
                                    new SecurityVulnerCompliance.GetMachineCompliance(main,securityTargetDetailsBean);
                            ...
                            display_results = SecurityLDAPUtils.getAssignedPolicy(SECURITY_SCAN_TYPE_OVAL, target.getName(), target.getType(), target.getId(), main, _user);
                            List<MachineBean> allMachineList = new SecurityVulnerCompliance.GetTargetMachine(main, target.getId()).getMachineBeanList();
                            ...
                            if( display_results != null && display_results.size() > 0 ) {
                                for (int i = 0; i<display_results.size(); i++) {
                                    ...
                                    MachineListBean machineListBean = new SecurityVulnerCompliance.GetGroupCompliance(main,securityTargetDetailsBean, target.getId()).getMachineListBean();
...
//                                }
                        ...
                        if( !hasReadPermission ){
	                        ...
	                        if(null == allMachineList || allMachineList.size() == 0) {
	                        	...
	                        	for(MachineBean machineBean : allMachineList) {
	                        		...
	                        		String overallComplianceLevel = new SecurityVulnerCompliance.GetOverallCompliance(main, target.getId(), machineBean.getMachineName(), machineBean.getMachineID()).getOverallComplianceLevel();
            ...
            if ((bean != null) && bean instanceof Exception) {
                ...
                if (DEBUG) {
                    System.out.println("Security Vulnerability TgtViewAction: critical exception found");
                }
