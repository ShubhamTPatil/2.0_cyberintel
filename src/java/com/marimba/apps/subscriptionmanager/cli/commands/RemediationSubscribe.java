...
import com.marimba.apps.subscription.common.objects.RemediationGroupChannel;
...
/**
 * Command for Subscibing Remediation group Patches
 *
 * @author      Venkatesh Jeyaraman
 * @version 	$Revision$, $Date$
 */

public class RemediationSubscribe extends SubscriptionCLICommand implements ISubscribe,ISubscriptionConstants {

    private SubscriptionCLIBase  sb;
    private Hashtable validTypes;
    private Hashtable validRemediationState;
    private Hashtable subStateMapping;
    private Hashtable validExemptFromBlackout;

    public RemediationSubscribe() {
        ...
        validRemediationState = new Hashtable();
        validRemediationState.put(PATCH_ASSIGN, "type");
        validRemediationState.put(PATCH_EXCLUDE, "type");
    ...
    public boolean run(Hashtable args) throws SystemException {
        ...
        isFailed = remediationsubscribe(args);
        return isFailed;
    }
     ...
     */
    private boolean remediationsubscribe(Hashtable args)
        ...
        //arg0
        if("-modify".equals((String) args.get("remedysubscribe:args" + i++))) {
            isModifyPolicy = true;
        } else {
        ...
        String dnSwitchOrTargetName = ((String) args.get("remedysubscribe:args" + i++)).toLowerCase();

        try {
            if ("-dn".equals(dnSwitchOrTargetName)) {
                // arg2 is name and arg3 is type if -modify is not given
                name = (String) args.get("remedysubscribe:args" + i++);
                type = (String) args.get("remedysubscribe:args" + i++);
            ...
            String url = null;
            while ((arg = (String) args.get("remedysubscribe:args" + i++)) != null) {
                ...
                if (!validRemediationState.containsKey(state1)) {
                ...
                if(sub.isChannelFound(url)) {
                    ...
                    //Create a new channel for a remediation patch group
                    //channel name is the title of the remediation patch group
                    String patchtitle = "";
                    if(url != null) {
                    ...
                    RemediationGroupChannel groupCh = new RemediationGroupChannel(url,patchtitle,state1,null,ORDER,null,null);
                    groupCh.setExemptFromBlackout(isExempt);
                    sub.addChannel(groupCh);
                }
            ...
            while ((arg = (String) args.get("remedysubscribe:args" + i++)) != null) {
                if ("-wowdep".equals(arg)) {
                    String wowDeployment = (String) args.get("remedysubscribe:args" + i++);
            ...
            logMsg(LOG_AUDIT_REMEDY_SUBSCRIBE_SAVE_SUCCESS, LOG_AUDIT, type + " " + name, CMD_SUBSCRIBE_REMEDY);
            return false;
        } catch (SystemException se) {
            printMessage(resources.getString("cmdline.subscribesavefailed") + " " + type + " " + name);
            logMsg(LOG_AUDIT_REMEDY_SUBSCRIBE_SAVE_FAILED, LOG_AUDIT, type + " " + name, CMD_SUBSCRIBE_REMEDY);
            throw se;
        } finally {
