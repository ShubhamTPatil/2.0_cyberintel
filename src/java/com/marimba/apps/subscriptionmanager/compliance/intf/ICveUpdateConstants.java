...
public interface ICveUpdateConstants {
  ...
  public String UPDATE_TABLE_SQL_OLD = "insert into security_cve_info \n" +
  ...
  public String UPDATE_TABLE_SQL = "truncate table security_cve_info;\n"
      ...
      + "case when v3_cvss > 0 then v3_cvss else v2_cvss end as cvss, \n"
      + "case when v3_cvss > 0 then v3_cvss else v2_cvss end as cvss , '-1','-1' ,'NA' ,'2.0','2.0' \n"
      + "from cve_info tp with (nolock) \n"
      + "where not exists (select 1 from security_cve_info ts with (nolock) where tp.cve_id = ts.cve_name);";
}
