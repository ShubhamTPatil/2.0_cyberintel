...
public final class PluginPreviewAction extends AbstractAction {
    ...
    private static final String[] configProps = {"title1", "pluginStatus" , "title2" , "publishurl", "publishurl.username", "publishurl.password", "publishurl.subscribeuser", "publishurl.subscribepassword",
            "title4", "securityinfo.url", "securityinfo.subscribeuser", "securityinfo.subscribepassword",
            "title5", "customscanner.url", "customscanner.subscribeuser", "customscanner.subscribepassword",
            "title6", "db.type", "db.hostname", "db.port", "db.name", "db.username", "db.password",
            "title7", "db.thread.min", "db.thread.max",
    ...
    private static final String[] subConfigProps = {"title1", "subscriptionmanager.pluginStatus" , "title2" , "subscriptionmanager.publishurl", "publishurl.username", "publishurl.password", "publishurl.subscribeuser", "publishurl.subscribepassword",
            "title4", "subscriptionmanager.securityinfo.url", "subscriptionmanager.securityinfo.subscribeuser", "subscriptionmanager.securityinfo.subscribepassword",
            "title5", "subscriptionmanager.customscanner.url", "subscriptionmanager.customscanner.subscribeuser", "subscriptionmanager.customscanner.subscribepassword",
            "title6", "subscriptionmanager.dbType", "subscriptionmanager.db.hostname", "subscriptionmanager.db.port","subscriptionmanager.db.username", "subscriptionmanager.db.password",
            "title7", "subscriptionmanager.db.thread.min", "subscriptionmanager.db.thread.max",
    ...
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        // Authentication Type property display in policy plugin preview page if AD directory service
        if ((vendor == null) || ! LDAPConstants.VENDOR_AD.equals(vendor)) {
...
//        }
    ...
    private void validateSettings(IMapProperty formbean, Vector errors, Locale locale) {
        ...
        if (!isValidDbName(getPropertyValue(formbean, "db.username"))) {
            errors.addElement(printMsg(locale, "errors.required", printMsg(locale, "errors.database.db.invalidUserName")));
        }
        if (!isValidDbName(getPropertyValue(formbean, "db.password"))) {
            errors.addElement(printMsg(locale, "errors.required", printMsg(locale, "errors.database.db.invalidPassword")));
        }
