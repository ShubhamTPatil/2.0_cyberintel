...
public final class SCAPSecuritySaveAction extends AbstractAction {
    HttpSession session;
    private ConfigProps config;
    public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
            throws ServletException {
        if (DEBUG) {
            System.out.println("SCAPSecuritySaveAction called");
        }
        init(request);
        session = request.getSession();
        loadStandardSCAPContentDetails();
        saveState(request, (SCAPSecurityForm) form);

        // and forward to the next action
        String forward = (String)((AbstractForm) form).getValue("forward");
        return (new ActionForward(forward, true));
    }
    private void saveState(HttpServletRequest request, SCAPSecurityForm form) throws GUIException {
        ...
        try {
            ...
            if(null == scapOption || "false".equals(scapOption)) {
                setValue(sub, scapProps, SCAP_SECURITY_OPTION, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_FILENAME, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_ID, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_TITLE, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_TEMPLATE_NAME, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDPROFILE_ID, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDPROFILE_TITLE, null, priority);
                setValue(sub, scapProps, SCAP_SECURITY_SCAP_REMEDIATION_ENABLED, null, priority);
                ...
                setValue(sub, scapProps, SCAP_SECURITY_OPTION, scapOption, priority);
                String selectedScapAndProfiles = (String) ((AbstractForm) form).getValue("selectedScapAndProfilesNonWindows");
                System.out.println("selectedScapAndProfilesNonWindows : " + selectedScapAndProfiles);
                ...
                if (null != selectedScapAndProfiles && !selectedScapAndProfiles.trim().isEmpty()) {
                    ...
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_FILENAME, parserBean.getContentFileNames(), priority);
                    ...
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_ID, contentIds, priority);
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_TITLE, parserBean.getContentTitles(), priority);
                    if("customize".equals(scapOption)) {
                    	setValue(sub, scapProps, SCAP_SECURITY_TEMPLATE_NAME, parserBean.getCustomTemplateNames(), priority);
                    } else {
                    	setValue(sub, scapProps, SCAP_SECURITY_TEMPLATE_NAME, null, priority);
                    }
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDPROFILE_ID, parserBean.getProfileIDs(), priority);
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDPROFILE_TITLE, parserBean.getProfileTitles(), priority);
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_REMEDIATION_ENABLED, remediateEnabled, priority);
                } else {
                    setValue(sub, scapProps, SCAP_SECURITY_OPTION, null, priority);
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_FILENAME, null, priority);
                    setValue(sub, scapProps, SCAP_SECURITY_SCAP_SELECTEDCONTENT_ID, null, priority);
    ...
    private void loadProfileConfig() {
        config = new ConfigProps(new File(main.getDataDirectory(), SCAP_SECURITY_FILE_NAME));
    }
    private Hashtable readProfile(String profileName) {
        System.out.println("SCAPSecuritySaveAction: reading profile: " + profileName);
