...
/**
 * This action is called when visiting the SCAP(Windows) setting page. It reads the SCAP option and populates the form for display.
 *
 * @author Selvaraj Jegatheesan
 */

public final class USGCBSecurityLoadAction extends AbstractAction {
    HttpSession session;
    ...
    public ActionForward execute(ActionMapping mapping,
        ...
        USGCBSecurityForm scapForm = ((USGCBSecurityForm) form);
        ...
        try {
            ...
            if(null != selectedProfileName && null != selectedType) {
                ...
                String scapOption = getPropertyValue(sub.getProperty(PROP_USGCB_SECURITY_KEYWORD, USGCB_SECURITY_OPTION));
                String priorityValue = getPriorityValue(sub.getProperty(PROP_USGCB_SECURITY_KEYWORD, USGCB_SECURITY_OPTION));
    ...
    private void loadCustomSCAPContentDetails() {
        ...
        for (int i = 0; i < scapProfileStr.length; i++) {
            if ("name".equals(scapProfileStr[i])) {
                ...
                while (st.hasMoreTokens()) {
                    ...
                    System.out.println("Custom template :" + templateName);
        ...
        session.setAttribute("usgcbprofiles", scapPropsVector);
        session.setAttribute("scapBeansListCustom", scapBeansListCustom.values());
        session.setAttribute("usgcbprofilespropsmap", JSONValue.toJSONString(customPropsMap));
    }
}
