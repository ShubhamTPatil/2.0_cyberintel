...
public class CustomSecurityProfileForm extends AbstractForm {
	String status;
	String statusDesc;
	String name;
	String description;
    ...
    public void loadProfile(Hashtable props) {
        System.out.println("CustomSecurityProfileForm:: Loading profile: " + props.get(CUSTOM_SECURITY_PROFILE_NAME));
        reset();
        System.out.println("CustomSecurityProfileForm:: Loading profile: after reset() method...");

        try {
            scapEnabled = (String) props.get(CUSTOM_SECURITY_ENABLED);
            name = (String) props.get(CUSTOM_SECURITY_TEMPLATE_NAME);
            description = (String) props.get(CUSTOM_SECURITY_TEMPLATE_DESC);
            forceApplyEnabled = (String) props.get(CUSTOM_SECURITY_FORCE_APPLY);
            selectedSCAPContent = (String) props.get(CUSTOM_SECURITY_SCAP_SELECTEDCONTENT_FILENAME);
            String selectedProfileId = (String) props.get(CUSTOM_SECURITY_SCAP_SELECTEDPROFILE_ID);
			if ((selectedSCAPContent != null) && (selectedProfileId != null)) {
				//ignore...
			} else {
				if (SCAPUtils.getSCAPUtils().getSupportedCustomContents() != null) {
					selectedSCAPContent = SCAPUtils.getSCAPUtils().getSupportedCustomContentsXML()[0];
					selectedProfileId = (String) (SCAPUtils.getSCAPUtils().getProfilesForScapContent(selectedSCAPContent).keySet().toArray()[0]);
				}
			}
			initialIFramePath = "/spm/securitymgmt?command=gethtml&target=custom&customize=true&content=" + selectedSCAPContent + "&profile=" + selectedProfileId;
        ...
        System.out.println("CustomSecurityProfileForm:: Loading profile: method leaving...");
    }
