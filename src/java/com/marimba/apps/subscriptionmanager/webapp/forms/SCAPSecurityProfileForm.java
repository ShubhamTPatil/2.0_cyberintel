...
public class SCAPSecurityProfileForm extends AbstractForm {
	String status;
	String statusDesc;
	String name;
	String description;
    ...
    public void loadProfile(Hashtable props) {
        System.out.println("SCAPSecurityProfileForm:: Loading profile: " + props.get(SCAP_SECURITY_PROFILE_NAME));
        reset();
        System.out.println("SCAPSecurityProfileForm:: Loading profile: after reset() method...");

        try {
            scapEnabled = (String) props.get(SCAP_SECURITY_ENABLED);
            name = (String) props.get(SCAP_SECURITY_TEMPLATE_NAME);
            description = (String) props.get(SCAP_SECURITY_TEMPLATE_DESC);
            forceApplyEnabled = (String) props.get(SCAP_SECURITY_FORCE_APPLY);
            selectedSCAPContent = (String) props.get(SCAP_SECURITY_SCAP_SELECTEDCONTENT_FILENAME);
            String selectedProfileId = (String) props.get(SCAP_SECURITY_SCAP_SELECTEDPROFILE_ID);
			if ((selectedSCAPContent != null) && (selectedProfileId != null)) {
				//ignore...
			} else {
                if (SCAPUtils.getSCAPUtils().getSupportedSCAPContents() != null) {
                    selectedSCAPContent = SCAPUtils.getSCAPUtils().getSupportedSCAPContentsXML()[0];
                    selectedProfileId = (String) (SCAPUtils.getSCAPUtils().getProfilesForScapContent(selectedSCAPContent).keySet().toArray()[0]);
                }
			}
			initialIFramePath = "/spm/securitymgmt?command=gethtml&target=nonwindows&customize=true&content=" + selectedSCAPContent + "&profile=" + selectedProfileId;
        ...
        System.out.println("SCAPSecurityProfileForm:: Loading profile: method leaving...");
    }
