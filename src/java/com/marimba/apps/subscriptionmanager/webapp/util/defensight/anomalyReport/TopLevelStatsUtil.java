...
public class TopLevelStatsUtil {
     ...
     * @return
     */
    public List<TopLevelStatsChartData> populateTopLevelStatsData(IConfig tunerConfig, CosmosPagedIterable<MachineNameList> machineList, OffsetDateTime currentTime, OffsetDateTime prevTime) {

        debugInfo( "populateTopLevelStatsData:: AnomalyUtil.populateTopLevelStatsData getting called.");

        getCosmosConnection(tunerConfig);

        HashMap<OffsetDateTime, OffsetDateTime> intervals = createEqualSixtyTimeIntervals(prevTime, currentTime);
        ...
        machineList.stream().forEach(machineName -> {
            CompletableFuture<List<TopLevelStatsChartData>> future = CompletableFuture.supplyAsync(() -> {
                ...
                debugInfo("populateTopLevelStatsData "+hostname+":: SELECT c.host.hostname as hostname, c.predictions as anomaly, c.event.created as time FROM c  WHERE c.event.created <= '"+currentTime.toString()+"' AND c.event.created >= '"+prevTime.toString()+"' AND c.host.hostname = '"+hostname+"' AND (c.predictions = 'true' OR c.predictions = 'Not_Trained') AND c.event.created != null");
                ...
                CosmosPagedIterable<TopLevelStatsChartData> result = executeQueryForTopLevelStats(new SqlQuerySpec(AnomalyConstants.Queries.TOP_LEVEL_STATS_FOR_HOST, paramList));

                debugInfo("populateTopLevelStatsData "+hostname+":: Query executed");
                debugInfo("START hostname ="+hostname);

                List<TopLevelStatsChartData> dataPoints = topLevelStatsDataPoints(result, hostname, intervals);
    ...
    private List<TopLevelStatsChartData> topLevelStatsDataPoints(
        ...
        return intervals.entrySet()
                ...
                .flatMap(entry -> {
                    ...
                    boolean hasTrue = filteredData.stream().anyMatch(data -> "true".equals(data.getAnomaly()));
                    boolean hasNotTrained = filteredData.stream().anyMatch(data -> "Not_Trained".equals(data.getAnomaly()));
                    ...
                    if (hasTrue) {
                        ...
                        return Stream.of(new TopLevelStatsChartData(hostname, endTime.toString(), "Not_Trained"));
                    } else {
                        return Stream.of(new TopLevelStatsChartData(hostname, endTime.toString(), "false"));
                    }
                })
